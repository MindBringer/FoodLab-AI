...
class ValidateRequest(BaseModel):
    schema_name: str = Field(..., examples=["tasks/document_analysis"])
...
def list_schemas() -> dict[str, Any]:
    ...
    for json_file in sorted(SCHEMA_BASE_DIR.rglob("*.json")):
        rel = json_file.relative_to(SCHEMA_BASE_DIR)
        schema_name = str(rel.parent).replace("\\", "/")
        items.setdefault(schema_name, []).append(json_file.stem)
    for key in items:
        items[key] = sorted(items[key])
    return {"schemas": items}


@app.get("/schemas/{schema_name:path}")
...
@app.get("/schemas/{schema_name:path}/{schema_version}")
